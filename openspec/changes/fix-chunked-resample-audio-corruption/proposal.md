...
- Code: `app/stt/segmenter.py` (`WebRTCUtteranceSegmenter`, the chunked-resampling fix), `app/audio/input.py` (settle-window fix), and `app/pipeline/orchestrator_async.py` (discard-first-utterance backstop, `DEBUG_SAVE_UTTERANCES` diagnostic, and — the actual dominant fix — correcting `input_sample_rate` from a hardcoded 16000 to the real capture rate). No changes to `app/stt/audio_utils.py` or `app/stt/vad.py`.
